...
CMRCBitmapButton::CMRCBitmapButton(){m_hBitmap = NULL;}
CMRCBitmapButton::~CMRCBitmapButton(){}
BOOL CMRCBitmapButton::Create(DWORD dwStyle,const RECT&rect,CWnd*pParentWnd,UINT nID)
{
	dwStyle |= BS_OWNERDRAW;return CButton::Create(NULL,dwStyle,rect,pParentWnd,nID);
}
...
BOOL CMRCBitmapButton::AutoLoad(UINT nID, CWnd * pParent, BOOL bSetSize)
{
	...
	if(!LoadBitmap(buttonName)){return FALSE;}
	if(bSetSize==TRUE){AutoSize();}return TRUE;
}
BOOL CMRCBitmapButton::AutoSize()
// sets the size of the button to match the loaded bitmap
{
	if (m_hBitmap == NULL)return FALSE;
	SetWindowPos(NULL,0,0,m_sizeImage.cx+6,m_sizeImage.cy+6,SWP_NOACTIVATE|SWP_NOMOVE|SWP_NOZORDER);
	return TRUE;
}
void DrawDisabled(CDC* pDC, CPoint pt, CSize sizeImage, HBITMAP hBitmap)
// draw a disabled bitmap borrowed from old toolbar code in MFC
{
	CDC monoDC;
	CBitmap bmp;
	// create a mono bitmap and select it into a DC compatible with that passed in
  // This DC is used to create the mask
  if(!monoDC.CreateCompatibleDC(pDC)||!bmp.CreateBitmap(sizeImage.cx, sizeImage.cy, 1, 1, NULL))return;
	CDC glyphDC;
