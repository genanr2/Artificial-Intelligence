...
BOOL CMRCColorLabel::Register()
{
    ...
    if (!::RegisterClass(&wc)){ASSERT(0);return FALSE;} 
    return TRUE;
}
LRESULT CALLBACK CMRCColorLabelWndProc(HWND hWnd,UINT uiMsg,WPARAM wParam,LPARAM lParam)
{
...
{
	static WNDPROC NEAR pfnSuper=NULL;return&pfnSuper;
}
...
CMRCColorLabel::~CMRCColorLabel(){delete m_pDib;delete m_pDibPal;}
...
void CMRCColorLabel::SetFont(CFont*pFont){m_pFont=pFont;InvalidateRect(NULL);}
...
void CMRCColorLabel::DoPaint(CDC * pDC)
{
	...
	{
		::DrawBorderSunkenOuter(pDC,&rect);::DrawBorderSunkenInner(pDC,&rect);
	}
