...
{
	CObArray arrWnd;GetFloatingBars(arrWnd);ArrangeWindows(arrWnd, dwOrient);
}
...
{
	DestroyDynamicBars();// remove bars allocated dynamically - we reload these at present
...
// ignores bars in the registry that do not exist on this run (MFC asserts otherwise)
{
	CMRCDockState state;
...
#ifdef _DEBUG
void CMRCMDIFrameWndSizeDock::AssertValid()const{CMDIFrameWnd::AssertValid();}
/*
void CMRCMDIFrameWndSizeDock::Dump(CDumpContext& dc)const
{
//	CMDIFrameWnd::Dump(dc);dc<<"\nCMRCMDIFrameWndSizeDock - dockbars";
}
*/
#endif
////////////////////////////////////////////////////////////////////////////////
...
//}}AFX_MSG_MAP
END_MESSAGE_MAP()
BOOL MRCWriteProfileBinary(LPCTSTR lpszSection,LPCTSTR lpszEntry,LPVOID pData,DWORD nBufferSize)
...
// for removing any value not just binary ones
{
	...
	{
		lRes=RegSetValueEx(hSecKey,lpszEntry,NULL,REG_BINARY,(LPBYTE)pData,nBufferSize);
	}
	RegCloseKey(hSecKey);
...
{
	afxData.UpdateSysColors();CMDIFrameWnd::OnSysColorChange();
}
void CMRCFrameWndSizeDock::OnSysColorChange() 
{
	afxData.UpdateSysColors();CFrameWnd::OnSysColorChange();
}
...
// this resizes the existing bars, giving progressively smaller bars. 
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            DockControlBar(pBar, dwMRCDockBarMap[i][0], &rect);return;
        }
    }
